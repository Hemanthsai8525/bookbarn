...
export default function Navbar() {
  ...
  const navLinks = [
    ...
    { name: "Browse Books", path: "/books", visible: true },
          ...
          <div className="flex justify-between items-center h-20"> {/* Taller navbar for premium feel */}
            ...
            <div className="hidden md:flex items-center gap-8">
              ...
              <div className="flex items-center gap-6">
                {/* Public & User Links */}
                {(!user || (isUser && !isAdmin && !isDelivery && !isVendor)) && (
                  <>
                    {navLinks.filter(l => l.visible).map(link => (
                      <Link
                        key={link.path}
                        to={link.path}
                        className={`text-gray-600 font-medium hover:text-amber-700 transition-colors ${location.pathname === link.path ? 'text-amber-700 font-bold' : ''}`}
                      >
                        {link.name}
                      </Link>
                    ))}
