...
export default function Register() {
  ...
  async function verifyAndRegister(otpValue) {
    ...
    try {
      ...
      const errorMsg = err.response?.data?.error || err.response?.data || "Invalid OTP or Registration failed";

      if (typeof errorMsg === 'string' && errorMsg.includes("Duplicate entry")) {
        alert("This email or phone number is already registered. Please use a different one or login.");
        window.location.reload(); // Redirects/reloads the registration page as requested
        return;
      }
