...
export default function ForgotPassword() {
    ...
    const handleResetPassword = async (e) => {
        ...
        let url = '';
        if (userType === 'user') url = '/user/reset-password';
        if (userType === 'vendor') url = '/vendors/reset-password';
        if (userType === 'delivery') url = '/delivery/reset-password';

        try {
            // Note: Backend reset-password endpoints need to be updated to accept email+password instead of token
            await api.post(url, { email, newPassword });
