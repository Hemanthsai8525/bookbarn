...
export default function DeliveryRegister() {
  ...
  async function sendOTP() {
    ...
    try {
      await axios.post("http://localhost:8080/otp/send-registration", {
        email: form.email
      });
  ...
  async function verifyAndRegister(otpValue) {
    ...
    try {
      // Verify OTP
      const verifyRes = await axios.post("http://localhost:8080/otp/verify", {
        email: form.email,
        otp: otpValue
      });
      ...
      if (verifyRes.data.verified) {
        ...
        await axios.post("http://localhost:8080/delivery/register", form);

        const toast = document.createElement("div");
        toast.className = "fixed top-5 right-5 bg-blue-600 text-white px-6 py-3 rounded-xl shadow-2xl z-[999] animate-fade-in flex items-center gap-2";
        toast.innerHTML = `<svg width="20" height="20" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path d="M20 6L9 17l-5-5"></path></svg> Delivery Agent Registration Successful!`;
        document.body.appendChild(toast);
  ...
  async function resendOTP() {
    setLoading(true);
    try {
      await axios.post("http://localhost:8080/otp/resend", {
        email: form.email,
  ...
  return (
    <div className="min-h-screen bg-[#FDFBF7] flex items-center justify-center p-4 relative overflow-hidden">
      ...
      <div className="absolute top-0 left-0 w-full h-full overflow-hidden z-0">
        ...
        <div className="absolute top-[10%] -right-[10%] w-[40%] h-[40%] bg-indigo-100/40 rounded-full blur-[100px]" />
      </div>
      ...
      >
        <div className="text-center mb-8">
          <div className="w-14 h-14 bg-blue-600 rounded-xl flex items-center justify-center text-white mx-auto shadow-lg shadow-blue-600/30 mb-4">
            <Truck size={28} />
          </div>
          <h2 className="text-3xl font-serif font-bold text-gray-900">
            {step === 1 ? "Join as Delivery Agent" : "Verify Email"}
          </h2>
          <p className="text-gray-500 mt-2">
            {step === 1 ? "Deliver books and earn with BookBarn" : `Enter the OTP sent to ${form.email}`}
          </p>
        </div>
          ...
          <form onSubmit={(e) => { e.preventDefault(); sendOTP(); }} className="space-y-4">
            ...
            <div className="space-y-1">
              <div className="relative group">
                <User className="absolute left-4 top-3.5 text-gray-400 group-focus-within:text-blue-600 transition-colors" size={18} />
                <input
                  value={form.name}
                  onChange={(e) => setForm({ ...form, name: e.target.value })}
                  placeholder="Full Name"
                  className="w-full pl-11 pr-4 py-3 rounded-xl bg-white/50 border border-gray-200 focus:border-blue-500 focus:ring-4 focus:ring-blue-500/10 outline-none transition-all text-sm"
                />
              </div>
            ...
            <div className="grid md:grid-cols-2 gap-4">
              ...
              <div className="space-y-1">
                <div className="relative group">
                  <Phone className="absolute left-4 top-3.5 text-gray-400 group-focus-within:text-blue-600 transition-colors" size={18} />
                  <input
                    value={form.phone}
            ...
            <div className="space-y-1">
              <div className="relative group">
                ...
                <input
                  ...
                  placeholder="Delivery Area (e.g., North Delhi)"
                  className="w-full pl-11 pr-4 py-3 rounded-xl bg-white/50 border border-gray-200 focus:border-blue-500 focus:ring-4 focus:ring-blue-500/10 outline-none transition-all text-sm"
                />
              </div>
              {errors.area && <p className="text-red-500 text-xs ml-1">{errors.area}</p>}
            </div>
            ...
            <button
              type="submit"
              disabled={loading}
              className="w-full mt-4 bg-blue-600 text-white font-bold py-3.5 rounded-xl shadow-lg shadow-blue-600/20 hover:bg-blue-700 hover:shadow-xl hover:-translate-y-0.5 transition-all flex items-center justify-center gap-2 disabled:opacity-70 disabled:cursor-not-allowed"
            >
        ...
        <p className="text-center text-gray-600 mt-8">
          Already a delivery agent?
          <Link to="/login" className="text-blue-700 font-bold hover:underline ml-1">Login here</Link>
        </p>
      </motion.div>
    </div>
  );
