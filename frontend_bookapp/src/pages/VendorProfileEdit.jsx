...
export default function VendorProfileEdit() {
    ...
    async function sendOTP() {
        setSendingOTP(true);
        try {
            await axios.post("http://localhost:8080/otp/send-reset", { email });
            setOtpSent(true);
            ...
            toast.innerHTML = `<svg width="20" height="20" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path d="M20 6L9 17l-5-5"></path></svg> OTP sent to ${email}`;
    ...
    async function verifyOTPAndUpdate(otpValue) {
        ...
        try {
            ...
            if (verifyRes.data.verified) {
                // OTP verified, update profile with password
                await updateProfile();
            }
        } catch (err) {
    ...
    async function updateProfile() {
        ...
        try {
            ...
            if (form.name) localStorage.setItem("vendorName", form.name);
            ...
            document.body.appendChild(toast);
            setTimeout(() => { toast.remove(); navigate("/vendor/profile"); }, 2000);
        } catch (err) {
            ...
            <div className="max-w-xl mx-auto">
                ...
                <div className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
                    <div className="px-8 py-6 border-b border-gray-100 bg-gray-50/50">
                        <h1 className="text-xl font-bold text-gray-900">Edit Business Profile</h1>
                        <p className="text-sm text-gray-500 mt-1">Update your vendor details</p>
                    </div>
